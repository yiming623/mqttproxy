...
int _mosquitto_handle_publish(struct mosquitto *mosq)
{
	...
	sendto(sockfd,message->msg.payload,message->msg.payloadlen,0,
		 (struct sockaddr *)&servaddr,sizeof(servaddr));
	close(sockfd);
	_mosquitto_log_printf(mosq, MOSQ_LOG_NOTICE,
			"Client %s received PUBLISH (d%d, q%d, r%d, m%d, '%s','%s', ... (%ld bytes))",
			mosq->id, message->dup, message->msg.qos, message->msg.retain,
			message->msg.mid, message->msg.topic,message->msg.payload,
